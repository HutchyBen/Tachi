...
describe("GET /api/v1/games/iidx/SP/songs/:songID", () => {
	...
	it("returns 200 with the song and its charts", async () => {
		const res = await mockApi.get(`/api/v1/games/iidx/SP/songs/${SONG_LEGACY_ID}`);
	...
	it("includes song searchTerms and altTitles", async () => {
		...
		const res = await mockApi.get(`/api/v1/games/iidx/SP/songs/${OTHER_LEGACY_ID}`);
	...
	it("only returns charts for the requested playtype", async () => {
		...
		expect(res.status).toBe(200);
		expect(res.body.body.charts).toHaveLength(1);
		expect(res.body.body.charts[0].playtype).toBe("SP");
	});
	...
	it("returns 400 when songID is not a positive integer", async () => {
	...
	it("returns 400 for an invalid game", async () => {
		const res = await mockApi.get(`/api/v1/games/notreal/SP/songs/${SONG_LEGACY_ID}`);
	...
	it("returns 400 for an invalid playtype", async () => {
		const res = await mockApi.get(`/api/v1/games/iidx/NOTREAL/songs/${SONG_LEGACY_ID}`);
	...
	it("returns 0 charts when the song has no charts for this playtype", async () => {
		...
		const res = await mockApi.get(`/api/v1/games/iidx/SP/songs/${BARE_LEGACY_ID}`);
