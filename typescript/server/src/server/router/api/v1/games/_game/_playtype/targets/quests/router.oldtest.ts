...
t.test("GET /api/v1/games/:game/:playtype/targets/quests", (t) => {
	...
	t.test("Should search quests.", async (t) => {
		await MONGODB_KILL.quests.insert([
...
t.test("GET /api/v1/games/:game/:playtype/targets/quests/:questID", (t) => {
	...
	t.beforeEach(async () => {
		await Promise.all([
			MONGODB_KILL["goal-subs"].insert(IIDXSPQuestGoalSubs),
			MONGODB_KILL.questlines.insert({
				questlineID: "set_id",
