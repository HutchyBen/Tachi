...
t.test("PATCH /api/v1/scores/:scoreID", (t) => {
	...
	t.test("Should modify the session if the user has permission to.", async (t) => {
		...
		const score = await MONGODB_KILL.scores.findOne({ scoreID: "TESTING_SCORE_ID" });
...
t.test("DELETE /api/v1/scores/:scoreID", (t) => {
	...
	t.test("Should delete a score if the requester can.", async (t) => {
		...
		const dbScore = await MONGODB_KILL.scores.findOne({ scoreID: "TESTING_SCORE_ID" });
	...
	t.test("Should allow admins to delete other's scores", async (t) => {
		...
		await MONGODB_KILL.scores.insert(someoneElsesScore);
		...
		t.resolveMatch(
			MONGODB_KILL.scores.findOne({ scoreID: someoneElsesScore.scoreID }),
