...
t.test("GET /api/v1/users/:userID/games/:game/:playtype/targets/goals", (t) => {
	...
	t.test("Should retrieve this user's goal subscriptions.", async (t) => {
		await MONGODB_KILL.goals.insert(dupedGoal);
		await MONGODB_KILL["goal-subs"].insert(dupedGoalSub);
...
t.test("POST /api/v1/users/:userID/games/:game/:playtype/targets/add-goal", async (t) => {
	...
	t.beforeEach(async () => {
		...
		await MONGODB_KILL["personal-bests"].insert(customScorePB);
	});
	...
	t.beforeEach(() => MONGODB_KILL.charts.iidx.insert(anotherFakeChart));

	// @ts-expect-error weird type issues
	t.beforeEach(() => MONGODB_KILL.folders.insert(dm(TestingIIDXFolderSP10, {})));
	...
	t.test("Should set valid goals.", async (t) => {
		...
		for (const input of validInput) {
			await t.test(`Valid Goal: ${input.caseName}`, async (t) => {
				...
				const existsInDb = await MONGODB_KILL.goals.findOne({
				...
				const subscribed = await MONGODB_KILL["goal-subs"].findOne({
					userID: 1,
...
t.test("GET /api/v1/users/:userID/games/:game/:playtype/targets/goals/:goalID", (t) => {
	...
	t.test("Should return parent quests if goal has any.", async (t) => {
		await MONGODB_KILL.goals.insert(IIDXSPQuestGoals);
		await MONGODB_KILL["goal-subs"].insert(IIDXSPQuestGoalSubs);
		await MONGODB_KILL.quests.insert(TestingIIDXSPQuest);
...
t.test("DELETE /api/v1/users/:userID/games/:game/:playtype/targets/goals/:goalID", async (t) => {
	...
	t.test("Should delete a goal subscription if subscribed.", async (t) => {
		...
		const dbRes = await MONGODB_KILL["goal-subs"].findOne({
			userID: 1,
	...
	t.test("Should reject a goal deletion if goal has parent quests.", async (t) => {
		await MONGODB_KILL.quests.insert(TestingIIDXSPQuest);
		await MONGODB_KILL["quest-subs"].insert(TestingIIDXSPQuestSub);
		await MONGODB_KILL.goals.insert(IIDXSPQuestGoals);
		await MONGODB_KILL["goal-subs"].insert(
