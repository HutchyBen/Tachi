...
import { type ChartDocument, IIDX_GRADES, IIDX_LAMPS, type UserDocument } from "tachi-common";
import t from "tap";
...
const SetFolders = async () => {
	...
	await MONGODB_KILL["personal-bests"].insert(deepmerge(TestingIIDXSPScorePB, {}));
};
...
t.test("PUT /api/v1/users/:userID/games/:game/:playtype/showcase", (t) => {
	...
	t.test("Requires the user to be authed as the requested user.", async (t) => {
		await MONGODB_KILL.users.insert(mkFakeUser(2));
		await MONGODB_KILL["api-tokens"].insert({
			userID: 2,
			identifier: "alt_token",
			permissions: {
	...
	t.test("Should replace a user's preferences.stats with the contained stats.", async (t) => {
		...
		const data = await MONGODB_KILL["game-settings"].findOne({
			userID: 1,
			game: "iidx",
			playtype: "SP",
		});
