...
t.test("#TachiScoreToServerScore", (t) => {
	...
	t.test("Should correctly convert a tachiScore to a serverScore", async (t) => {
		await MONGODB_KILL.scores.insert(mockScoreDocument);
...
t.test("#CreatePOSTScoresResponseBody", async (t) => {
	...
	const chartDoc = (await MONGODB_KILL.charts.usc.findOne()) as ChartDocument<
	...
	t.test("Should correctly return POSTScoresResponseBody", async (t) => {
		...
		await MONGODB_KILL["personal-bests"].insert(uscScorePBsSet);
		await MONGODB_KILL.users.insert(mockUserDocs);
