import { SDVX_IMPL } from "#game-implementations/games/sdvx";
...
const baseMetrics: MongoProvidedMetrics["sdvx"] = {
	lamp: "CLEAR",
	score: 9_003_000,
};

const scoreData: ScoreData<"sdvx"> = {
	lamp: "CLEAR",
	score: 9_700_000,
	grade: "AAA",
	judgements: {},
...
async function seedAlbidaChart() {
...
async function insertSdvxScoreRow(opts: {
	scoreData: ScoreData<"sdvx">;
	scoreId: string;
	timeAchievedMs: number;
	userId: number;
}) {
	const { data, derived, judgements } = mongoScoreDataToPg("sdvx", opts.scoreData);
...
describe("SDVX_IMPL", () => {
	describe("chartSpecificValidators.exScore", () => {
		it("accepts non-negative values", () => {
			expect(SDVX_IMPL.chartSpecificValidators.exScore(1000, chart)).toBe(true);
			expect(SDVX_IMPL.chartSpecificValidators.exScore(0, chart)).toBe(true);
		});

		it("rejects negative EX score", () => {
			expect(SDVX_IMPL.chartSpecificValidators.exScore(-1, chart)).toBe(
				"EX Score must be non-negative. Got -1",
			);
		});

		it.skip("SDVX EXScore should be upper bounded on maximum EX Score", () => {
			// not supported yet (see sdvx.ts TODO)
		});
	});
	...
	describe("scoreDeriver (grade)", () => {
		const g = (score: number, expected: string) =>
			expect(
				SDVX_IMPL.scoreDeriver(dmf(baseMetrics, { score }) as never, chart as never).grade,
			).toBe(expected);
	...
	describe("goal formatters", () => {
		const mockScore = mkMockScore("sdvx", chart, scoreData);
	...
	describe("PB merging (CreatePBDoc)", () => {
		beforeEach(seedAlbidaChart);

		it("joins best lamp into the default-metric PB", async () => {
			const { id: userId } = await seedUser({ username: "sdvx_pb_lamp" });
			const main = mkMockScore("sdvx", chart, scoreData);

			const lampScoreData: ScoreData<"sdvx"> = {
				...scoreData,
				score: 0,
				lamp: "ULTIMATE CHAIN",
				enumIndexes: {
					...scoreData.enumIndexes,
					lamp: SDVX_LAMPS.ULTIMATE_CHAIN,
				},
			};

			await insertSdvxScoreRow({
				userId,
			...
			const pb = await CreatePBDoc("sdvx", userId, chart, log);
		...
		it("joins best optional exScore from the lamp PB source", async () => {
			const { id: userId } = await seedUser({ username: "sdvx_pb_ex" });
			const main = mkMockScore("sdvx", chart, scoreData);
