...
import type { Int16, Int32 } from "./numerics";
...
export const SCORE_GAP_EXTENTION = -1;
export const BONUS_BOUNDARY = SCORE_MATCH / 2;
export const BONUS_NON_WORD = SCORE_MATCH / 2;
export const BONUS_CAMEL_123 = BONUS_BOUNDARY + SCORE_GAP_EXTENTION;
export const BONUS_CONSECUTIVE = -(SCORE_GAP_START + SCORE_GAP_EXTENTION);
...
function bonusAt(input: Array<Rune>, idx: number): Int16 {
	if (idx === 0) {
		return BONUS_BOUNDARY;
	}
...
function trySkip(input: Array<Rune>, caseSensitive: boolean, char: Rune, from: number): number {
...
function isAscii(runes: Array<Rune>) {
...
function asciiFuzzyIndex(input: Array<Rune>, pattern: Array<Rune>, caseSensitive: boolean): number {
...
export const fuzzyMatchV2: AlgoFn = (
	...
	let B: Int16Array | null = null;
	let C0: Int16Array | null = null;
	let F: Int32Array | null = null;
	let H0: Int16Array | null = null;
	let offset16 = 0;
	let offset32 = 0;
	...
	let pchar = pattern[0];
	let prevH0 = toShort(0);
	let prevCharClass = Char.NonWord;
	let inGap = false;
	const Tsub = T.subarray(idx);
	const H0sub = H0.subarray(idx).subarray(0, Tsub.length);
	const C0sub = C0.subarray(idx).subarray(0, Tsub.length);
	const Bsub = B.subarray(idx).subarray(0, Tsub.length);
	...
	const [, C] = alloc16(offset16, slab, width * M);
	...
	for (const [off, f] of Fsub.entries()) {
		...
		const pidx = off + 1;
		const row = pidx * width;
		const Tsub = T.subarray(f, lastIdx + 1);
		const Bsub = B.subarray(f).subarray(0, Tsub.length);
		const Csub = C.subarray(row + f - f0).subarray(0, Tsub.length);
		const Cdiag = C.subarray(row + f - f0 - 1 - width).subarray(0, Tsub.length);
		const Hsub = H.subarray(row + f - f0).subarray(0, Tsub.length);
		const Hdiag = H.subarray(row + f - f0 - 1 - width).subarray(0, Tsub.length);
		const Hleft = H.subarray(row + f - f0 - 1).subarray(0, Tsub.length);

		Hleft[0] = 0;

		for (const [off, char] of Tsub.entries()) {
			const col = off + f;
			let consecutive: Int16 = 0;
			let s1: Int16 = 0;
			let s2: Int16 = 0;
	...
	if (withPos && pos !== null) {
		...
		while (true) {
			...
			preferMatch =
				C[I + j0] > 1 || (I + width + j0 + 1 < C.length && C[I + width + j0 + 1] > 0);
			j--;
		}
	}
...
function calculateScore(
	...
	for (let idx = sidx; idx < eidx; idx++) {
		...
		if (rune === pattern[pidx]) {
			...
			if (inGap) {
				score = score + SCORE_GAP_EXTENTION;
			} else {
...
export const suffixMatch: AlgoFn = (
	...
	if (
		...
		trimmedLen = trimmedLen - whitespacesAtEnd(text);
	}
