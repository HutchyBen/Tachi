...
async function FastUpdateSessions() {
	...
	const newScoreIDRefs: Array<{ new: string; old: string }> = await monkDB
		.get("temp-update-map")
		.find({});
...
function MoveLamps(
	scoreData: { lamp?: string } & ScoreData<"chunithm:Single">,
...
const migration: Migration = {
	...
	up: async () => {
		...
		await EfficientDBIterate(
			...
			db.scores,
			(score: { scoreData: { lamp?: string } } & ScoreDocument<"chunithm:Single">) => {
		...
		const updatedGoalIDs: Array<{ goal: GoalDocument; new: string; old: string }> = await monkDB
