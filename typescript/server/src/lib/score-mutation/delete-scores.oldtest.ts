...
t.test("#DeleteScore", (t) => {
	...
	t.test("Should delete a score and everything pertaining to it", async (t) => {
		...
		await MONGODB_KILL.imports.insert(mockImportDocument);
		await MONGODB_KILL.sessions.insert(mockSessionDocument);

		await MONGODB_KILL.scores.insert([score, score2]);

		// This function doesn't return anything, instead,
		// we need to check external state.
		await DeleteScore(score);

		const dbScore = await MONGODB_KILL.scores.findOne({
			scoreID: "scoreid_1",
		});

		t.equal(dbScore, null, "Should remove the score from the database.");

		const dbImport = await MONGODB_KILL.imports.findOne({
			importID: "mockImportID",
		});
		...
		const dbSession = await MONGODB_KILL.sessions.findOne({
			sessionID: "mockSessionID",
		});
	...
	t.test(
		...
		async (t) => {
			...
			await MONGODB_KILL.scores.insert(score);
			...
			const dbScore = await MONGODB_KILL.scores.findOne({ scoreID: score.scoreID });
	...
	t.test("Should destroy its parents if this killed the only child", async (t) => {
		...
		await MONGODB_KILL.imports.insert(
		...
		await MONGODB_KILL.sessions.insert(
	...
	t.test("Should update classes if the user's classes should need to change.", async (t) => {
		await MONGODB_KILL.charts.sdvx.insert(
		...
		const res = await MONGODB_KILL["game-stats"].findOne({
			userID: 1,
			game: "sdvx",
			playtype: "Single",
		});
