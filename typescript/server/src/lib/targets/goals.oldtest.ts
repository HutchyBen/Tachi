		...
		t.beforeEach(async () => {
			...
			await MONGODB_KILL["personal-bests"].insert(
				dmf(TestingIIDXSPScorePB, {
					scoreData: { lamp: "CLEAR", enumIndexes: { lamp: 4 } },
...
t.test("#GetRelevantFolderGoals", (t) => {
	...
	t.beforeEach(async () => {
		...
		await MONGODB_KILL.folders.insert(TestingIIDXFolderSP10);
		await MONGODB_KILL.folders.insert(sp11folder);
		await MONGODB_KILL.goals.insert(fakeFolderGoalDocument);
		await MONGODB_KILL.goals.insert(notFolderGoalDocument);
		await CreateFolderChartLookup(TestingIIDXFolderSP10, true);
...
t.test("#GetRelevantGoals", (t) => {
	...
	t.beforeEach(async () => {
		...
		await MONGODB_KILL.songs.iidx.insert(
			GetKTDataJSON("./tachi/tachi-songs-iidx.json") as Array<SongDocument<"iidx">>,
		);

		const lotsOfCharts = await MONGODB_KILL.charts.iidx.find({}, { limit: 20 });
		...
		await MONGODB_KILL.goals.insert(goals);
	...
	t.test(
		...
		async (t) => {
			...
			const ourCharts = [
				...(await MONGODB_KILL.charts.iidx.find({}, { limit: 5 })),
				...(await MONGODB_KILL.charts.iidx.find({}, { skip: 20, limit: 5 })),
			];
