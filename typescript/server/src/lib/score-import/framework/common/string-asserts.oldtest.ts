...
t.test("#AssertStrAsPositiveInt", (t) => {
	...
	t.strictSame(
		astr("1.4"),
		new InvalidScoreFailure(`err (Not an integer -- 1.4.)`),
		"Should reject float input",
	);

	t.strictSame(
		astr("NaN"),
		new InvalidScoreFailure(`err (Not an integer -- NaN.)`),
		"Should reject NaN",
	);
	t.strictSame(
		astr("-0.3"),
		new InvalidScoreFailure(`err (Not an integer -- -0.3.)`),
		"Should reject negative float input",
	);

	t.strictSame(
		astr("0xFF"),
		new InvalidScoreFailure(`err (Not an integer -- 0xFF.)`),
		"Should reject hex input",
	);

	t.strictSame(
		astr("0b11"),
		new InvalidScoreFailure(`err (Not an integer -- 0b11.)`),
		"Should reject binary input",
	);

	t.strictSame(
		astr("0o77"),
		new InvalidScoreFailure(`err (Not an integer -- 0o77.)`),
		"Should reject oct input",
	);

	t.strictSame(
		astr("--1"),
		new InvalidScoreFailure(`err (Not an integer -- --1.)`),
		"Should reject double neg input",
	);

	t.strictSame(
		astr("12f"),
		new InvalidScoreFailure(`err (Not an integer -- 12f.)`),
	...
	t.strictSame(
		astr("-1"),
		new InvalidScoreFailure(`err (Was negative -- -1.)`),
