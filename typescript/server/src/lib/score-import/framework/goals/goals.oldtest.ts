...
t.test("#UpdateGoalsForUser", (t) => {
	...
	t.test("Should correctly update goals when user achieves goal.", async (t) => {
		await MONGODB_KILL.goals.insert(baseGoalDocument);

		await MONGODB_KILL["goal-subs"].insert(baseGoalSubscriptionDocument);
		...
		const r = await MONGODB_KILL["goal-subs"].findOne({ goalID: "FAKE_GOAL_ID", userID: 1 });
	...
	t.test("Should correctly update goals when user does not achieve goal.", async (t) => {
		...
		await MONGODB_KILL.goals.insert(goal);
		...
		await MONGODB_KILL["personal-bests"].insert(
			dmf(TestingIIDXSPScorePB, { scoreData: { score: 1 } }),
		);
...
t.test("#ProcessGoal", (t) => {
	t.beforeEach(ResetDBState);
	t.beforeEach(async () => {
		await MONGODB_KILL.goals.insert(HC511Goal);
	});

	t.test("Should process the users goal if a score has changed.", async (t) => {
		await MONGODB_KILL["goal-subs"].insert(HC511UserGoal);
	...
	t.test("Should unset wasInstantlyAchieved if the goal became unachieved.", async (t) => {
		...
		await MONGODB_KILL["goal-subs"].insert(achievedGoalSub);
	...
	t.test("Should return undefined if the progress has not changed.", async (t) => {
		...
		await MONGODB_KILL["goal-subs"].bulkWrite([firstUpdate!.bwrite]);

		const goalSub = await MONGODB_KILL["goal-subs"].findOne({
			userID: 1,
			goalID: HC511Goal.goalID,
		});
