...
t.test("#ConverterIRFervidex", (t) => {
	...
	t.test("Should turn DP randoms into tuples.", async (t) => {
		...
		const Testing511DPA = dmf(Testing511SPA, {
			// @ts-expect-error lolol
			playtype: "DP",
			chartID: "dp_test",
			data: {},
		});

		await MONGODB_KILL.charts.iidx.insert(Testing511DPA);
	...
	t.test("Should highlight existing scores.", async (t) => {
		...
		await MONGODB_KILL.scores.insert(fakeScore);
		...
		const dbRes = await MONGODB_KILL.scores.findOne({ scoreID: fakeScore.scoreID });
