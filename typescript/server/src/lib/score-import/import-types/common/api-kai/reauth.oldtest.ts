...
t.test("#CreateKaiReauthFunction", (t) => {
	...
	t.beforeEach(async () => await MONGODB_KILL["kai-auth-tokens"].remove({}));
	...
	t.test("Should create a working reauthentication for the service.", async (t) => {
		...
		await MONGODB_KILL["kai-auth-tokens"].insert(deepmerge(authDoc, {}));
		...
		const dbChange = await MONGODB_KILL["kai-auth-tokens"].findOne({
			userID: 1,
			service: "FLO",
		});
